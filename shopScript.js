import { products } from "./shopItems.js";
...
for (let index = 0; index < products.length; index++) {
    const { id, productName, productPrice, productImg, detail1, detail2 } = products[index]
    itemContainer.innerHTML +=
        ...
        `<p class="itemDetail1">${detail1}</p>` +
        `<p class="itemDetail2">${detail2}</p>` +
        ...
        products.map((item) => {
            ...
            if (cartArray.includes(id)) {
                ...
                return eachCartItemContainer.innerHTML +=
                    `<div class="eachCart">` +
                    ` <img src="${productImg}" class="cartImg" alt="">` +
                    `<div class="cartDesc">` +
                    `<h1 class="cartItemName">${productName}</h1>` +
                    `<p class="cartItemPrice">Rs. ${productPrice}</p>` +
